...
const downloadMaintenaceBill = async (req, res) => {
  try {
    ...
    if (!payments.length)
      return res.status(404).json({ success: false, message: "No paid records found" });
    ...
    const buildingName = admin?.bname || "Society";
    ...
    // Receipt label — first payment ka receiptNo use karo
    const receiptNo = payments[0].receiptNo
      ? `RCP-${String(payments[0].receiptNo).padStart(4, "0")}${isBulk ? " (BULK)" : ""}`
      : `#${String(payments[0]._id).slice(-8).toUpperCase()}`;
    ...
    const labelValue = (label, value, y) => {
      doc.fontSize(10).font("Helvetica").fillColor("#555555").text(label, MARGIN + 10, y);
      doc.fontSize(10).font("Helvetica-Bold").fillColor("#111111").text(value, MARGIN + 190, y);
    };
    ...
    const sectionHeader = (title, y) => {
      ...
      doc.fontSize(9).font("Helvetica-Bold").fillColor("#EAB308").text(title, MARGIN + 10, y + 6);
    };
    ...
    doc.fontSize(10).font("Helvetica-Bold").fillColor("#1a1a2e")
      .text("MAINTENANCE RECEIPT", (W - 200) / 2, 82, { align: "center", width: 200 });

    // ══ META ROW ══════════════════════════════════
    const metaY = 148;
    const boxW  = (W - MARGIN * 2 - 20) / 3;
    ...
    metaBox("RECEIPT NO",    receiptNo,   MARGIN);
    metaBox("BILLING MONTH", monthsLabel, MARGIN + boxW + 10);
    metaBox("STATUS",        "PAID ✓",   MARGIN + (boxW + 10) * 2, "#16a34a");
    ...
    [
      ...
      if (idx % 2 === 0) doc.rect(MARGIN, rowY, W - MARGIN * 2, 22).fill("#fafafa");
    ...
    [
      ...
      if (idx % 2 === 0) doc.rect(MARGIN, payRowY, W - MARGIN * 2, 22).fill("#fafafa");
    ...
    if (isBulk) {
      ...
      payments.forEach((p, idx) => {
        if (idx % 2 === 0) doc.rect(MARGIN, brkY, W - MARGIN * 2, 22).fill("#fafafa");
        doc.fontSize(10).font("Helvetica").fillColor("#555555").text(p.month, MARGIN + 10, brkY + 5);
        doc.fontSize(10).font("Helvetica-Bold").fillColor("#111111")
          .text(`Rs. ${Number(p.amount).toLocaleString("en-IN")}`, MARGIN + 190, brkY + 5);
        brkY += 22;
      });
      ...
      doc.fontSize(10).font("Helvetica-Bold").fillColor("#1a1a2e")
        .text(`Rs. ${totalAmount.toLocaleString("en-IN")}`, MARGIN + 190, brkY + 5);
      brkY += 22;
    ...
    doc.rect(MARGIN, amtY, W - MARGIN * 2, 64).fill("#FFFBEB").stroke("#FDE68A");
    doc.fontSize(11).font("Helvetica").fillColor("#92400E")
      .text("Total Amount Paid", MARGIN + 16, amtY + 12);
    doc.fontSize(26).font("Helvetica-Bold").fillColor("#92400E")
      .text(`Rs. ${totalAmount.toLocaleString("en-IN")}`, MARGIN + 16, amtY + 28);
    ...
    doc.fontSize(9).font("Helvetica-Oblique").fillColor("#888888")
      .text("This is a computer generated receipt and does not require a physical signature.",
        0, footerY + 16, { align: "center", width: W });
    doc.fontSize(10).font("Helvetica-Bold").fillColor("#1a1a2e")
    ...
    if (!res.headersSent)
      return res.status(500).json({ success: false, message: error.message || "Bill generation failed" });
    res.end();
  }
};
