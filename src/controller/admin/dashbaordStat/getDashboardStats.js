...
const getDashboardStats = async (req, res) => {
  try {
    ...
    if (!buildingCode || !month) {
      return res.status(400).json({ success: false, message: "Building code or month missing" });
    }
    ...
    const [members, payments, expenses, monthRecord] = await Promise.all([
      memberModel.find({ buildingCode, approvalStatus: "Approved", isVerified: true }).lean(),
...
const membersSummary = {
  flatOwners:  members.filter(m => m.memberType === "Flat" && m.memberStatus === "Owner").length,
  flatRenters: members.filter(m => m.memberType === "Flat" && m.memberStatus === "Tenant").length,
  shopOwners:  members.filter(m => m.memberType === "Shop" && m.memberStatus === "Owner").length,
  shopRenters: members.filter(m => m.memberType === "Shop" && m.memberStatus === "Tenant").length,
  total: members.length,
    ...
    const totalCollection = paidPayments.reduce((s, p) => s + (p.amount || 0), 0);
